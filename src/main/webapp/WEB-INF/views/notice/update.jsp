...
function list(page){
   location.href="update?curPage="+page+"&searchOption-${map.searchOption}"+"&keyword=${map.keyword}";
}
</script>
         ...
         $.ajax({
            ...
            success: function(data) {
               ...
               if(checkImageType(data)){ 
                  ...
                  str = "<div><a href='update/upload/displayFile?fileName="+getImageLink(data)+  "'>";
                  str += "<img src='update/upload/displayFile?fileName="+data+"'></a>";
               // 이미지 파일이 아니면 다운로드
               } else { 
                  str = "<div><a href='update/upload/displayFile?fileName="+data+"'>"+getOriginalName(data)+"</a>";   
               }
...
<style type="text/css">
   @font-face{
    ...
    body {
             font-family: SDream2, sans-serif;
            }
...
<div class="col-md-12">                
   <div id="updatelist">      <!-- 자유게시판 리스트 시작-->   
                  ...
                  <form name="form1" method="post" action="update" class="navbar-form navbar-right">
                     <select name="searchOption" class="form-control">
                        <!-- 검색조건을 검색처리후 결과화면에 보여주기위해  c:out 출력태그 사용, 삼항연산자 -->
                        <option value="all" <c:out value="${map.searchOption == 'all'?'selected':''}"/> >전체</option>
                        <option value="update_writer_id" <c:out value="${map.searchOption == 'update_writer_id'?'selected':''}"/> >이름</option>
                        <option value="update_content" <c:out value="${map.searchOption == 'update_content'?'selected':''}"/> >내용</option>
                        <option value="update_title" <c:out value="${map.searchOption == 'update_title'?'selected':''}"/> >제목</option>
                     </select>
                         ...
                         <tr>
                          ...
                          <c:forEach items="${map.list }" var="bean"> 
                            ...
                            <tr>
                               ...
                               <td class="td" style="width: 6%; "><a class="detailList" href="update/detail?idx=${bean.update_seq }">${bean.update_seq }</a></td>
                               <td class="td" style="width: 60%; "><a class="detailList" href="update/detail?idx=${bean.update_seq }">
                               ${bean.update_title }           
                               <c:if test="${bean.update_file_path1 != null}">
                               		 <span class="glyphicon glyphicon-paperclip"></span>
                               </c:if>		 
                               		 </a></td>
                               <td class="td"><a class="detailList" href="update/detail?idx=${bean.update_seq }">${bean.update_writer_id }</a></td>
                               <td class="td" style="width: 10%; "><a class="detailList" href="update/detail?idx=${bean.update_seq }">
                                     <span id="date"><fmt:formatDate value="${bean.regi_date }" pattern="yyyy-MM-dd"/></span> 
                        </a></td>
                               <td class="td" style="width: 6%; "><a class="detailList" href="update/detail?idx=${bean.update_seq }">${bean.update_hits }</a></td>
                            </tr>
                         </c:forEach>
...
<div class="col-md-12"> 
	...
	<div id="updateadd">   <!-- 글쓰기 페이지 -->
	     ...
	     <form id="form1" target="iframePhoto" action="upload" method="post" enctype="multipart/form-data"> -->   
	      ...
	      <div>
	            <input type="text" name="update_title" class="form-control" placeholder="제목을 입력해주세요">
	            
	         </div>
	         <div>
	            <textarea id="summernote" class="summernote" name="update_content"  rows="" cols="" style="height: 500px;"></textarea>
	         </div>
