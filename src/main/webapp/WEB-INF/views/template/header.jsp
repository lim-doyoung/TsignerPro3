...
<link rel="stylesheet" type="text/css" href="<%=root %>/css/tsigner.css" />
...
// 		}
	...
	function login() {
		...
		var temp = window.location.search.substring(1); //url에서 처음부터 '?'까지 삭제
		...
		$.ajax({
			...
			success : function(data) {
				if (data == 'logoutSuccess') {
					...
					alert('로그아웃 실패로 다시 로그아웃을 시도합니다.');
					window.location.href = "http://192.168.1.8:8080/tsigner/logoutUser";
					window.location.href = "http://192.168.1.8:8080/tsigner";
				}
			},
			error : function(request, status, error) {
		...
		<div class="modal-dialog modal-sm" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="row">
					<!-- 카카오톡 로그인 -->
					<div id="loginApi" class="" style="text-align:center; border: none;">
							<a href="https://kauth.kakao.com/oauth/authorize?client_id=337e2a1093fbc87f1c0fa2bea4366df1&redirect_uri=http://192.168.1.8:8080/tsigner/kakao&response_type=code">
								<img height="40" alt="카카오 로그인 버튼" src="<%=root %>/imgs/kakaotalklogo.jpg">
							</a>
							&nbsp;
							<!-- 네이버 로그인 -->
							<!-- 네이버아이디로로그인 버튼 노출 영역 -->
							...
							<a href="<%=apiURL%>"><img height="40"
								src="<%=root %>/imgs/naverlogopng.png" /></a>
							&nbsp;
							<!-- 페이스북 로그인 무늬만 있음 -->
							<a href="popup();"><img height="40"
								src="<%=root %>/imgs/facebooklogo.jpg" /></a>
							<br><br>
					</div>
					</div>
					</div>
									...
									<ul class="nav navbar-nav navbar-right">
										<c:choose>
											...
											if(profile.startsWith("/profile/")||profile.startsWith("profile/")){
												System.out.println("뭘까 왜: "+profile);
												System.out.println("여기로오긴함");
											%>
...
<%-- 											<li ><img width="50px" height="50px" style="margin-left: 0px" id="profile" alt="" src="<%=root %>/${sessionScope.profile }"></li> --%>
	...
	<div class="view intro-2" style="background: url('<%=root%>/imgs/bok6.png') no-repeat center;">
		<div class="full-bg-img">
			<div class="mask rgba-black-light flex-center">
				<div class="container text-center white-text">
					<div class="white-text text-center wow fadeInUp">
						<h2 class="textLocation1" style="color: black"></h2>
						<p class="textLocation2"></p>
					</div>
				</div>
			</div>
		</div>
